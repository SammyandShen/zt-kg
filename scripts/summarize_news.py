...
- 幂等：已有 brief 的 (code, trade_date) 跳过；--force 重写
...
def summarize_date(conn, claude_bin: str, d: str, force: bool) -> tuple[int, int]:
    rows = conn.execute(
        "SELECT e.code, e.name, e.reason_type FROM limit_up_events e WHERE e.trade_date=?"
        + ("" if force else " AND (e.code, e.trade_date) NOT IN "
                            "(SELECT code, trade_date FROM briefs)")
