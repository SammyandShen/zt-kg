...
**📖 项目说明书 = ARCHITECTURE.md（2026-08-06 用户定）**：架构/背景/表结构/ER关系/
全部指标口径的权威文档。**每次功能迭代前先读它的相关章节；迭代改动表结构、
口径、数据流、文件职责时，必须在同一次迭代中同步更新它**（阈值校准另记
semantic_config.calibration_log，前端口径同步 METRIC_DEFS）。本文件只保留
操作约定+变更日志；与说明书冲突时以代码为准并立即修正文档。
...
- **个股页K线与交易概览（2026-08-05）**：日K线（120交易日，MA5/10/20+量柱+
  涨停▲/炸板△标注+十字悬浮）数据在 `docs/data-kline.js`（~13MB，个股页打开时
  script 注入懒加载，file:// 兼容；build_site 随每日班次重生成，全部涨停股×120日
  对齐全局日期轴，停牌位 null）。概览8块：最新收盘/近20日、流通市值、涨停战绩、
  连板能力、**本股隔日溢价**（stock_stats 导出：均值/高开率/样本数，对照全市场
  基线）、首封习惯（竞价/早盘/午前/午后型）、封单力度（均封单+封成比）、
  龙头履历（担任 rank1 / rank2-3 的轮次数）。
